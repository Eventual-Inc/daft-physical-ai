...
def test_window_rates_match_numpy_computer_per_episode() -> None:
    ...
    frames = frame_table(episodes).select(
...
def test_state_only_tables_get_state_only_rates() -> None:
    frames = frame_table({0: make_state(4, seed=4)}).select(
        "episode_index", "frame_index", state_frame_features(col("observation.state"))
    )
...
def test_scenario_predicates_as_column_expressions() -> None:
    ...
    frames = frame_table({0: state}).select(
        "episode_index", "frame_index", state_frame_features(col("observation.state"))
    )

    lifted = (
        add_temporal_features(frames, fps=30.0)
        .where(col("wrist_vert_vel_L") >= 0.20)
        .select("frame_index")
        .to_pydict()
    )
...
def test_forearm_roll_requires_both_rotation_columns() -> None:
    frames = frame_table({0: make_state(3, seed=6)}).select(
        "episode_index", "frame_index", state_frame_features(col("observation.state"))
    )
    with_axis = frames.with_column(
        "forearm_axis_L", col("wrist_rot6d_L").apply(lambda r: [1.0, 0.0, 0.0], return_dtype=daft.DataType.list(daft.DataType.float64()))
    )
