...
def encode_playable_stream(kind: str) -> np.ndarray:
    ...
    try:
        ...
        # PyAV types add_stream() as any stream kind; only a video stream can be
        # asked for a width or fed a VideoFrame.
        stream = cast("Any", container.add_stream(codec, rate=10))
