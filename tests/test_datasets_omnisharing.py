...
import daft_physical_ai.datasets as datasets_module
from daft_physical_ai.datasets import omnisharing
from daft_physical_ai.datasets.omnisharing import (
...
def test_objects_probes_max_slots_and_pads_the_rest(tmp_path):
    root = tmp_path / "objs"
    ...
    # Slots beyond an episode's object count are empty, not zero-filled.
    assert np.asarray(two["obj3"]).size == 0
    assert two["obj3.name"] is None
    assert np.asarray(none["obj1"]).size == 0
...
def test_objects_max_objects_pads_beyond_the_widest_episode(tmp_path):
    root = tmp_path / "pad"
    write_df2_episode(root / episode_filename(1, "212953", 93, 110056), n_frames=4, n_objects=1)

    # Padding a release wider than any episode present must read cleanly: a slot
    # nothing fills would otherwise be an all-null tensor column, which Daft
    # cannot build.
    (row,) = omnisharing.objects(omnisharing.raw(str(root)), max_objects=3).to_pylist()
    assert row["n_objects"] == 1
    assert np.asarray(row["obj1"]).shape == (4, omnisharing.OBJECT_POSE_WIDTH)
    assert np.asarray(row["obj3"]).size == 0
...
def test_module_is_registered_on_the_datasets_package():
    assert datasets_module.omnisharing is omnisharing
    assert "omnisharing" in datasets_module.__all__
