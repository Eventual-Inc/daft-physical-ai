...
and then deriving everything else from the executed copy:
  ...
  3. walk it once to build the markdown's per-code-cell outputs - image cells are
     ...
     fenced as text, and the interactive `.show()` HTML table is skipped (it has
     no readable markdown form);
...
_FALLBACK_REPR = re.compile(r"^<IPython\.core\.display\.\w+ object>$|^<Figure size .*>$")
...
def _markdown_output(cell: dict, image_dir: Path) -> str:
    """The one markdown output slot for a code cell (image link, stream text, or none)."""
    image_link = ""
    stream_text = ""
    for out in cell.get("outputs", []):
    ...
    # image wins (it's the demo's payoff); else stream text; else nothing (skip HTML tables)
    return image_link or stream_text.rstrip()
