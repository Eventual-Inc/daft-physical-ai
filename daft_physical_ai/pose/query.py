...
def writing_grip(t: dict, thr: dict) -> np.ndarray:
...
def hammer_grip(t: dict, thr: dict) -> np.ndarray:
...
def scenario_mask(
    pose: str | Callable[..., np.ndarray],
    tracks_by_tag: dict[str, dict],
    ...
    **kwargs: object,
    ...
    for tag in tags:
        hand_mask = np.asarray(scenario(tracks_by_tag[tag], thresholds or {}, **kwargs), dtype=bool)  # ty: ignore[call-top-callable]
...
def segments_of(
    frames: Sequence[int],
    gap_merge: int = SEG_GAP_MERGE,
    min_frames: int = SEG_MIN_FRAMES,
) -> list[tuple[int, int]]:
    ...
    start, prev = sorted_frames[0], sorted_frames[0]
...
def top_segments(frames: Sequence[int], max_segments: int = MAX_SEGS) -> list[tuple[int, int]]:
