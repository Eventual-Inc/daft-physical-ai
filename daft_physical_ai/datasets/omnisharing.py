"""Lazy access to the `PX OmniSharing DB <https://huggingface.co/datasets/paxini/Omnisharing_DB_SampleData>`_.

PX OmniSharing is PaXini's omnimodal embodied-AI dataset: synchronized
...
| DF-3   | LeRobot Dataset v2.1 | *(none)*        | Use `daft.datasets.lerobot` instead           |

Note:
    The published OmniSharing data is licensed **CC-BY-NC-SA 4.0**
    (non-commercial), so this package neither downloads nor redistributes it.
    The toolkit that produces it lives at
...
def _require_episode_column(episodes: DataFrame, column: str = "episode") -> None:
    if column not in episodes.column_names:
        raise ValueError(
            ...
            f"(as produced by daft_physical_ai.datasets.omnisharing.raw). "
...
def describe(episodes: DataFrame) -> DataFrame:
    ...
    Args:
        episodes: Episode-level DataFrame from `raw`. Filter or ``limit`` it
...
def episode_metadata(episodes: DataFrame) -> DataFrame:
    ...
    Args:
        episodes: Episode-level DataFrame from `raw`.
...
def raw(
    ...
    `daft.Hdf5File` handle per episode. Pass the result to `trajectory`,
    `tactile` and friends to pull modalities out of the files.
    ...
    Examples:
        >>> from daft_physical_ai.datasets import omnisharing
        >>> episodes = omnisharing.raw("paxini/Omnisharing_DB_SampleData")  # doctest: +SKIP
...
def trajectory(
    ...
    *,
    ...
    Note:
        ...
        at ``i + 1``, with the final frame repeated). Use `frames` if you need
        that alignment materialized per row.

    Args:
        episodes: Episode-level DataFrame from `raw`. Filter it first: every
...
def tactile(
    ...
    *,
    ...
    def _read_tactile(file: Hdf5File) -> dict[str, Any]:
        ...
        # Daft cannot build a list column whose every value is null, so an
        # episode with no tactile at all reports empty layout lists.
        for side in selected:
            out[f"{side}.tactile.sensor_names"] = []
            out[f"{side}.tactile.sensor_lengths"] = []
        with file._open_h5py() as h5:
            for side in selected:
...
def audio(
    ...
    *,
    ...
    Note:
        ...
        ``txt`` attribute that `episode_metadata` surfaces as ``instruction``.
        No audio codec is needed to read it.
...
def objects(
    ...
    *,
    ...
    columns, and leaves the surplus empty for episodes with fewer objects.
    ...
    Returns:
        ...
        For an episode without that object slot the pose is an empty tensor and
        the three metadata columns are null; ``n_objects`` still reports how many
        objects the episode really has. When no episode has any objects, only
        ``n_objects`` is added.
    ...
    def _read_objects(file: Hdf5File) -> dict[str, Any]:
        ...
        out: dict[str, Any] = dict.fromkeys(return_fields)
        out["n_objects"] = 0
        # An unfilled slot's pose is an empty tensor rather than null, because
        # Daft cannot build a tensor column whose every value is null -- which
        # is exactly what padding the schema past the widest episode produces.
        for i in range(1, slots + 1):
            out[f"obj{i}"] = np.empty((0,), dtype="float32")
        with file._open_h5py() as h5:
            observation = _get(h5, _h5path("observation"))
...
def camera_payloads(
    episodes: DataFrame,
    cameras_: Sequence[str | tuple[str, str]] | str,
) -> DataFrame:
    ...
    for payloads this module cannot decode. Use `camera_frames` when you want
...
def camera_frames(
    ...
    *,
    ...
    Returns:
        ...
        codec is not decodable yield null frames; use `camera_payloads` to get
    ...
    *,
    ...
    Args:
        ...
        cameras_: RGBD camera name or names, e.g. ``"RGBD_0"``. Unlike
            `camera_frames` these are camera names only: depth is stored once
            per camera rather than per colour/left/right sub-stream.
    ...
    Returns:
        ...
        Cameras without an ``aligned_depth`` dataset yield an empty tensor and
        an empty index list.
        ...
        with file._open_h5py() as h5:
            for name in names:
                ...
                if node is None or not node.shape or node.shape[0] == 0:
                    # Absent or empty depth must not fail the whole episode.
                    # Emptiness is expressed as an empty array and an empty list
                    # rather than as null, because Daft cannot build a tensor or
                    # list column whose every value is null.
                    out[f"{name}.depth"] = np.empty((0,), dtype="uint16")
                    out[f"{name}.depth_frame_indices"] = []
                    continue
...
def stereo_extrinsics(
    ...
    which `cameras` passes through as an opaque string. The useful part is
    ``left_to_color``: the 4x4 transform from the left eye's frame into the
    colour camera's frame, needed to project the stereo pair into a common
    frame.

    Note:
        This is the *intra*-camera transform, distinct from the ``extrinsics``
        dataset that `cameras` reports, which places the whole camera relative
        to a reference rig (``RGBD_0`` for depth cameras, ``RGB_Camera6`` for
        colour ones).
...
def frames(
    ...
    *,
    ...
    Args:
        episodes: Episode-level DataFrame from `raw`. Filter it first.
