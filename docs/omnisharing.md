...
`daft_physical_ai.datasets.omnisharing` reads the DF-2 HDF5 stage directly, streaming only the datasets you ask for. Episodes are 0.4-3.2 GB each, so nothing is downloaded up front.
...
| DF-3 | LeRobot v2.1 | *(none)* | Use Daft's native `daft.datasets.lerobot` reader instead |
...
> **Warning: the depth unit is undocumented.** Unlike the tactile and joint datasets, `aligned_depth` carries no attributes at all, so nothing in the file lets a scale be derived. Values are returned exactly as stored — confirm with PaXini before treating them as millimetres.
...
Object tracks come from the toolkit's optional pose-estimation stage, so many episodes have none. `objects()` probes for the widest episode and pads the rest with empty poses, keeping the schema stable while `n_objects` still reports each episode's true count:
...
The reader needs no extras: `daft-physical-ai` already depends on `daft[hdf5,video]`, which brings h5py for the HDF5 episodes and PyAV for decoding camera payloads.
